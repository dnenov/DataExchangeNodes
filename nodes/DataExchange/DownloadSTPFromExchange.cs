...
namespace DataExchangeNodes.DataExchange
{
    ...
    public static class DownloadSTPFromExchange
    {
        ...
        /// <summary>
        /// Gets the Client instance using the same method as other nodes
        /// </summary>
        private static object TryGetClientInstance(List<string> diagnostics)
        {
            ...
            if (client == null)
            {
                try
                {
                    var clientType = Type.GetType("Autodesk.DataExchange.Client, Autodesk.DataExchange");
                    if (clientType != null)
                    {
                        diagnostics?.Add($"  Found Client type: {clientType.FullName}");
                        var staticFields = clientType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
                        var instanceField = staticFields.FirstOrDefault(f =>
                            f.FieldType == clientType ||
                            f.FieldType.IsAssignableFrom(clientType));
                        ...
                        if (instanceField != null)
                        {
                            ...
                            if (client != null)
                            {
                                diagnostics?.Add($"✓ Found Client instance via static field: {instanceField.Name}");
                            }
                        }
                    }
                    
                    // If still not found, search loaded assemblies for Autodesk.DataExchange.Client implementing IClient
                    if (client == null)
                    {
                        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                        {
                            try
                            {
                                var foundClientType = assembly.GetTypes().FirstOrDefault(t =>
                                    t.Name == "Client" &&
                                    t.Namespace == "Autodesk.DataExchange" &&
                                    typeof(IClient).IsAssignableFrom(t));
